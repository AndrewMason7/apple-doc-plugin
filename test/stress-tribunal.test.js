...
test('Tribunal Stress 1: HTTP Error Logging does not expose API key in URL or error output', async () => {
  ...
  try {
    const search = new GeminiSemanticSearch(secretKey, 'models/stress-model', baseUrl);
    const res = await search.embedQuery('stress query');
    assert.strictEqual(res, null);
    assert(!capturedLog.includes(secretKey), 'Secret API key must never appear in logs or error messages');
    assert.strictEqual(search.isCircuitOpen(), true, '500 status should trip circuit breaker');
  } finally {
...
test('Tribunal Stress 3: Malicious and Adversarial FTS5 Syntax Attacks', () => {
  ...
  for (const q of adversarialQueries) {
    ...
    }, `Query "${q}" must not throw syntax or execution errors`);
  }
...
test('Tribunal Stress 4: Corrupted Metadata Resilience', () => {
  ...
  for (const badPlatforms of corruptions) {
    // @ts-ignore
    db['db'].prepare("UPDATE symbols SET platforms = ? WHERE id = 'broken-meta'").run(badPlatforms);
...
test('Tribunal Stress 5: Concurrent Hybrid Searches under Circuit Breaker', async () => {
  ...
  assert(t1 - t0 < 500, `50 concurrent searches should complete in <500ms, took ${t1 - t0}ms`);
