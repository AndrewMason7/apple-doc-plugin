...
test('deserializeFloat32Array handles unaligned buffer offsets safely', () => {
	...
	const unalignedSubarray = rawBytes.subarray(
		oddOffset,
		oddOffset + source.byteLength,
	);

	assert.strictEqual(
		unalignedSubarray.byteOffset % 4,
		3,
		'Must be unaligned offset',
	);

	// Direct Float32Array constructor would throw RangeError:
	assert.throws(() => {
		new Float32Array(
			unalignedSubarray.buffer,
			unalignedSubarray.byteOffset,
			source.length,
		);
	}, /start offset/);
...
test('AppleDocsDB safely handles corrupted platform JSON without crashing', () => {
	...
	db['db']
		.prepare(
			"UPDATE symbols SET platforms = '{malformed json' WHERE id = 'corrupted-sym'",
		)
		.run();
