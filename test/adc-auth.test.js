...
test('ADC: Uses Bearer token when no API key is provided and ADC is available', async () => {
	...
	assert.deepStrictEqual(headers1, {
		Authorization: 'Bearer adc-access-token-12345',
	});
...
test('ADC: near-expiry credential is not cached for the original expires_in lifetime', async () => {
	let fetches = 0;
	const mockAuth = {
		getClient: async () => ({
	...
	const first = await search.getAuthHeaders();
	const second = await search.getAuthHeaders();
	assert.deepStrictEqual(first, { Authorization: 'Bearer short-lived-token' });
	assert.deepStrictEqual(second, first);
	assert.strictEqual(
		fetches,
		2,
		'expiry_date 30s out must win over expires_in of 3600s, so the 60s cache skew refetches',
	);
});
