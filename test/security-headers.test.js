...
test('GeminiSemanticSearch trips circuit breaker automatically on 429 response', async () => {
	...
	try {
		...
		assert.strictEqual(
			hitCount,
			1,
			'Server should not be hit when circuit is open',
		);
	} finally {
		server.close();
	}
...
test('GeminiSemanticSearch trips circuit breaker on 401 and 403 status codes', async () => {
	for (const status of [401, 403]) {
		const server = http.createServer((req, res) => {
			...
			res.end(
				JSON.stringify({ error: { message: 'Unauthorized / Forbidden' } }),
			);
		});
		...
		try {
			...
			assert.strictEqual(
				search.isCircuitOpen(),
				true,
				`Circuit breaker should trip on status ${status}`,
			);
		} finally {
			server.close();
		}
...
test('GeminiSemanticSearch trips circuit breaker on network failure (ECONNREFUSED)', async () => {
	...
	assert.strictEqual(
		search.isCircuitOpen(),
		true,
		'Circuit breaker should trip on network connection error',
	);
});
