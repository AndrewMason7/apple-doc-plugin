    ...
    return async (args) => {
        ...
        if (searchEngine) {
            ...
            if (platform) {
                ...
                filtered = filtered.filter((r) => r.platforms.length === 0 ||
...
const tryExactSymbolMatch = async (client, activeTechnology, query, queryMode, platform, symbolType) => {
    ...
    try {
        ...
        if (platform) {
            const platforms = data.metadata?.platforms?.map((item) => item.name).filter(Boolean) ??
                [];
            if (!platforms.some((item) => item.toLowerCase().includes(platform.toLowerCase()))) {
                return undefined;
            }
        }
