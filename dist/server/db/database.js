...
export function deserializeFloat32Array(buf) {
...
export class AppleDocsDB {
    ...
    queryLikePattern(pattern, framework, limit = 20) {
        let sql = `
        ...
        try {
            ...
            if (combined.length === 0 && tokens.length > 1) {
                const orQuery = tokens
                    .map((t) => `"${t.replace(/\*+$/, '')}"*`)
                    .join(' OR ');
                const orParams = [orQuery];
                if (framework) {
                    orParams.push(framework);
                }
                orParams.push(limit);
                try {
                    const orRows = this.db.prepare(sql).all(...orParams);
